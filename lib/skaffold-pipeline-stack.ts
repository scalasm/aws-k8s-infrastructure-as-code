...
export interface SkaffoldPipelineStackProps extends NestedStackProps {
  ...
  /**
   * Kubernetes cluster name (e.g., 'dev-cluster')
   */
...
export class SkaffoldPipelineStack extends NestedStack {
  ...
  constructor(scope: Stack, id: string, props: SkaffoldPipelineStackProps) {
    ...
    // [...] we define our pipeline. It has a source Action targeting the CodeCommit repository, two build Actions using
    // the previously defined projects, and finally a deploy Action that uses AWS CloudFormation. It takes the template
    // generated by the AWS CDK build Project (stored in the LambdaStack.template.json file), passes it to AWS
    // CloudFormation for deployment. To make the Lambda build output is an input to the AWS CloudFormation action, we
    // pass it in the extraInputs property.
